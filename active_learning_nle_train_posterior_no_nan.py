...
def train_per_network(num_network, nb_networks, inference_methods, density_estimators, round, num_rounds, training_batch_size, max_num_epochs, theta, x, task, neural_net, num_posterior_samples_per_network, filename):
    
    print(f"################ Network {num_network+1}/{nb_networks} ################")       
    density_estimator = inference_methods[num_network].append_simulations(
        theta, x, from_round=round
    ).train(
    ...
    ##MCMC
    posterior_sampling(task=task,nb_networks=nb_networks, 
                    density_estimator=density_estimator, num_posterior_samples_per_network=num_posterior_samples_per_network,
                        filename=filename)
    
    
    
    torch.save(density_estimator.state_dict(),f"state_dict_network_{num_network+1}_round_{round+1}_{neural_net}.pt")

def train_sample_networks(networks_ensemble, task, num_simulations, num_rounds, num_posterior_samples, simulation_batch_size, training_batch_size,prop_new_theta, neural_net_name,model):
    ...
    num_posterior_samples_per_network = int(num_posterior_samples/len(networks_ensemble))


    #mm inference pour tous  les networks
    inference_methods = []
    #state_dict = torch.load("state_dict_round_1_mdn.pt")
    ...
    for r in range(num_rounds):
        ...
        density_estimators = []
        filename = f"{model}_{num_posterior_samples}_posterior_samples_{len(networks_ensemble)}_networks_{neural_net_name}_{r+1}_rounds_{num_simulations}_ntrain.csv"
        if r>=3:
            with open(filename, "w",newline='') as file:
                writer = csv.writer(file)
                writer.writerow(["0", "1", "label"])
                file.close()
        #dict_per_round = {"round":r+1}
        for i in range (len(networks_ensemble)):
            ...
            ##MCMC
            if r>=3:
                posterior_sampling(task=task,nb_networks=len(networks_ensemble), 
                            density_estimator=density_estimator, num_posterior_samples_per_network=num_posterior_samples_per_network,
                                filename=filename)
            
            
            #dict_per_round[f"network_{i}"]= density_estimators[i].state_dict()
            #torch.save(density_estimator.state_dict(),f"state_dict_network_{i+1}_round_{r+1}_{neural_net_name}.pt")
            # Parallel(n_jobs=10, verbose=10)(delayed(train_per_network)(num_network=i, nb_networks=len(networks_ensemble), inference_methods=inference_methods,
            #                   density_estimators=density_estimators, round=r, num_rounds=num_rounds,training_batch_size=training_batch_size, 
            #                   max_num_epochs=max_num_epochs,theta=theta, x=x, task=task, neural_net=neural_net_name,
            #                   num_posterior_samples_per_network=num_posterior_samples_per_network, filename=filename) for i in range (len(networks_ensemble)))
            #dict_per_round[f"optimizer_networks_{i}"]=inference_methods[i].optimizer
        #torch.save(dict_per_round, f"state_dict_round_{r+1}_{neural_net_name}.pt")
...
def posterior_sampling(task, nb_networks, density_estimator, num_posterior_samples_per_network, filename):
