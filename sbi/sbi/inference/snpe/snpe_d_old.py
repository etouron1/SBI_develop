...
class SNPE_D(PosteriorEstimator):
    def __init__(
        ...
        kwargs = del_entries(locals(), entries=("self", "__class__", "regularisation"))
    ...
    def _loss(
        ...
        with torch.enable_grad():
            ...
            def logq(theta_batch):
                ...
                #     last_proposal[i] = self._proposal_roundwise[i].log_prob(theta)

                # log_proposal = torch.logsumexp(input=last_proposal, dim=0)
                # log_proposal = -torch.log(torch.tensor([self._round+1])) + log_proposal
                log_proposal = self._proposal_roundwise[-1].log_prob(theta_batch)
                return torch.sum(self._neural_net.log_prob(theta_batch, x) + log_proposal - self._prior.log_prob(theta_batch))
            
            # if self._round>0:
            #     print(self._proposal_roundwise[-1].log_prob(theta))
                
            #     print(grad(torch.sum(self._proposal_roundwise[-1].log_prob(theta)), theta))
            d_log_q_d_theta_sum = grad(logq(theta), theta, create_graph=True)
        
            first_deriv_sum = torch.sum(d_log_q_d_theta_sum[0]**2, dim=1)
           
            #second_deriv_sum = torch.zeros(theta.size(0), device=theta.device)
            laplacian = torch.zeros(theta.size(0), device=theta.device)
            regularization = torch.zeros(theta.size(0), device=theta.device)
            for i in range (theta.size(1)):
                #laplacian = grad(torch.sum(d_log_q_d_theta_sum[0], dim=0)[i], new_theta, create_graph=True)[0][:,i]
                second_deriv_sum = grad(torch.sum(d_log_q_d_theta_sum[0], dim=0)[i], theta, create_graph=True)[0][:,i]
                laplacian += second_deriv_sum
                regularization += second_deriv_sum**2
                

            loss_sum = laplacian + 0.5*first_deriv_sum + lmbda*regularization
