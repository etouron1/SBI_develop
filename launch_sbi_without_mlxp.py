...
def sample_posterior(args):
    ...
    filename = f"{args.model}_{args.nb_obs}_obs_{args.n_sequential}_rounds_ntr_{args.n_train}_np_{args.n_posterior}_"
    ...
    #snle(task=task, neural_net=args.neural_net_snle, num_samples=args.n_posterior, num_observation=args.nb_obs, num_simulations=args.n_train, num_rounds=args.n_sequential)
    torch.manual_seed(0)
    snpe(task=task, neural_net=args.neural_net_snpe, variant="C", num_samples=args.n_posterior, num_observation=args.nb_obs, num_simulations=args.n_train, num_rounds=args.n_sequential,  training_batch_size=args.batch_size_training)
...
def plot_metrics():
    ...
    dataframe_no_resampling = pd.read_csv("accuracy_1_obs_prior.csv")
    n_rounds=dataframe_no_resampling['round']
    c2st_no_resampling = dataframe_no_resampling['c2st']
    mmd_no_resampling = dataframe_no_resampling['mmd']
    dataframe_resampling = pd.read_csv("accuracy_1_obs_no_resampling.csv")
   
    c2st_resampling = dataframe_resampling['c2st']
    mmd_resampling = dataframe_resampling['mmd']
    plt.figure(figsize=(20,7))
    plt.suptitle(f"Comparison of performances")
    plt.subplot(121)
    plt.plot(n_rounds, c2st_no_resampling,marker='o',markerfacecolor = 'red', markeredgecolor = 'red', linestyle='--', color = "salmon", label="prior")
    plt.plot(n_rounds, c2st_resampling,'o',markerfacecolor = 'green', markeredgecolor = 'green',  linestyle='--', color = "lightgreen", label="proposal")
    plt.xlabel('nb of rounds')
    plt.ylabel('c2st')
    plt.legend()
    plt.subplot(122)
    plt.plot(n_rounds, mmd_no_resampling,'o',markerfacecolor = 'red', markeredgecolor = 'red', linestyle='--', color = "salmon", label="prior")
    plt.plot(n_rounds, mmd_resampling,'o',markerfacecolor = 'green',markeredgecolor = 'green', linestyle='--', color = "lightgreen", label="proposal")
    plt.xlabel('nb of rounds')
    plt.ylabel('mmd')
    plt.legend()
    plt.show()
...
def main(args):
    ...
    # plot_comparison('c2st_gaussian_mixture_mdn.csv')
    #plot_comparison('c2st_gaussian_mixture_default_method.csv')
    #plot_comparison('c2st_two_moons_best_method.csv')
